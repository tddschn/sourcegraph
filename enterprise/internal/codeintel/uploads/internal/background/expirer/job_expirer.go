...
func (s *expirer) HandleExpiredUploadsBatch(ctx context.Context, metrics *ExpirationMetrics, cfg *expirationConfig) (err error) {
...
func (s *expirer) handleRepository(ctx context.Context, repositoryID int, cfg *expirationConfig, now time.Time, metrics *ExpirationMetrics) error {
...
func (s *expirer) buildCommitMap(ctx context.Context, repositoryID int, cfg *expirationConfig, now time.Time) (map[string][]policies.PolicyMatch, error) {
	var (
		offset   int
